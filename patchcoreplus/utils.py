...
def plot_segmentation_images(
    ...
    for image_path, mask_path, anomaly_score, segmentation in tqdm.tqdm(
        ...
        savename_0 = image_path.split("/")
        savename_0 = "_".join(savename_0[-save_depth:])
        print("66 savename_0: ", savename_0)

        savename_path = os.path.normpath(image_path)
        savename_path = savename_path.split(os.sep)

        suffix = os.path.join(*savename_path[-2:])

        savepath = os.path.join(savefolder, suffix)
        print("68 savepath: ", savepath)
        ...
        os.makedirs(os.path.split(savepath)[0], ok_exists=True)
        # f.savefig(savepath)
        print(f"Saved the image to {savepath}")
        # plt.close()
